...
class RightHandSwitchBlockTest extends PropSpec with Matchers with PropertyChecks with MockitoSugar {
  ...
  property("trackGroups of RightHandSwitchBlock for direct route should return direct track group") {
    val tg = block.trackGroups(0)
    tg should have size (1)
    tg.head should have size (2)
  }

  property("tracksForJunction of RightHandSwitchBlock for direct route from 0 junction should return forward direct track") {
    val list = block.tracksForJunction(0)(0)
    list should have size (1)
    checkForSegment(list(0), entryPoint, dirPoint)
  }

  property("tracksForJunction of RightHandSwitchBlock for direct route from 1 junction should return backward direct track") {
    val list = block.tracksForJunction(0)(1)
    list should have size (1)
    checkForSegment(list(0), dirPoint, entryPoint)
  }

  property("tracksForJunction of RightHandSwitchBlock for direct route from 12 junction should return empty") {
    block.tracksForJunction(0)(2) shouldBe empty
  }

  property("trackGroups of diverging RightHandSwitchBlock should return diverging track group") {
    val tg = block.trackGroups(1)
    tg should have size (1)
    tg.head should have size (4)
  }

  property("tracksForJunction of diverging RightHandSwitchBlock from 0 junction should return forward diverging track") {
  ...
  property("tracksForJunction of diverging RightHandSwitchBlock from 1 junction should return empty tracks") {
    block.tracksForJunction(1)(1) shouldBe empty
  }

  property("tracksForJunction of diverging RightHandSwitchBlock from 2 junction should return backward diverging track") {
